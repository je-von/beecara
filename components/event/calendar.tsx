...
const CalendarEventView = ({ initialDate, events }: Props) => {
  ...
  useEffect(() => {
    ...
    let _prevActiveTotalDay = new Date(
      activeDate.getFullYear(),
      activeDate.getMonth(),
      0
    ).getDate();
    let _activeTotalDay = new Date(
      activeDate.getFullYear(),
      activeDate.getMonth() + 1,
      0
    ).getDate();
    ...
    for (let i = _activeStartingDay; i > 0; i--) {
      if (
        _active.getFullYear() === today.getFullYear() &&
        _active.getMonth() === today.getMonth()
      ) {
        _calendarDays.push({
          day: undefined,
          date: undefined,
          ...
          <div className="flex flex-col gap-4">
            {events
              ?.filter(
                (ev) =>
                  ev.startDate &&
                  isEqualDate(ev.startDate?.toDate(), activeDate)
              )
              .map((en, index) => {
